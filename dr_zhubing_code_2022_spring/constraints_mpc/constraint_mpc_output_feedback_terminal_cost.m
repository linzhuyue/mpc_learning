...
for i=1:duration_time/step_len
    bin1=-I_Nc*F*x_hat(:,i)+[x1_terminal_upper;x2_terminal_upper];
    Ain1=I_Nc*Phi;
    bin2=I_Nc*F*x_hat(:,i)-[x1_terminal_lower;x2_terminal_lower];
    Ain2=-I_Nc*Phi;
    Ain=[Ain1;Ain2];
    bin=[bin1,bin2];

    H=Phi'*QQ*Phi+RR;
    U=quadprog((H+H')/2,x_hat(:,i)'*F'*QQ*Phi,Ain,bin,[],[],lb,ub);
    %     u(:,i)=-K_mpc*x(:,i);
